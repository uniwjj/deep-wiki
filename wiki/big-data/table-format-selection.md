...
详见 [[iceberg]] 的四格式对比表。核心差异在"娘家"与侧重（来源 v3 分水岭）：
...
来源 腾讯落地实践 给出腾讯内部的真实选型逻辑（比对比表更有说服力）：
...
来源 数仓还是湖仓圆桌 是少见的多方视角材料——Apache Iceberg（Amoro PPMC）、Apache Hudi、Apache Paimon、StarRocks 四位社区代表同台讨论选型。核心共识：没有万能格式，按业务场景（实时性/更新频率/查询模式/生态绑定）选择，且四格式在互相借鉴融合（如 Iceberg v3 引入 CDC 能力、Paimon 借鉴 Iceberg 元数据设计）。
