...
来源 [[paimon-core-principle|千千寰宇核心原理]] 梳理了流计算+存储的演进：
...
Paimon 的诞生源于对现有湖格式的判断（来源 [[paimon-core-principle|千千寰宇]]）。当时需要的是：
...
来源 [[paimon-event-sourcing|事件溯源视角]] 提供了理解 Paimon 的独特角度——它不是"又一个兼容 Hive 的数据湖"，而是**第一个把"变更流"作为核心抽象的存储格式**，可称为**流原生存储（Stream-Native Storage）**。
...
Paimon 常被宣传为"基于 LSM 树"，但这容易误解（来源 [[paimon-event-sourcing|事件溯源视角]]）。它实际是 **LSM-Inspired**：
...
来源 [[paimon-flink-advanced|核心原理和Flink应用进阶]] 补充的关键概念：
...
来源 [[paimon-starrocks-streaming-lakehouse|Paimon+StarRocks 流式湖仓]] 总结的三大能力：
...
来源 [[paimon-starrocks-streaming-lakehouse|Paimon+StarRocks]] 给出典型流式湖仓分层架构：
...
Paimon 偏 Flink、Streaming First；Iceberg 中立开放；Hudi 偏 Hadoop；Delta 偏 Spark。详细对比见 [[iceberg]] 的四格式定位表。Paimon 与 Gravitino 协同的元数据治理方案见 [[paimon-gravitino-collaboration|Gravitino 与 Paimon 协同]]（待 Gravitino 主题摄取）。
...
- [[gravitino]] — Apache Gravitino 统一元数据（与 Paimon 协同）*待建*
