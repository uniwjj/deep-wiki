...
Iceberg 2017 年由 Netflix 的 Ryan Blue 和 Dan Weeks 创建，2018 年捐给 Apache，2020 年毕业为顶级项目。其诞生是为解决 Apache Hive 表的三个根本缺陷（来源 [[iceberg-wikipedia|Wikipedia]]）：
...
一次读取最新快照数据的过程（来源 [[iceberg-query-principle|底层数据查询原理]]）：
...
v3 的意义不止特性，更在于它让 Iceberg 真正成为**行业公共标准**——一年内 Snowflake、Databricks、Google BigQuery、Dremio、AWS 集体跟进落地（来源 [[iceberg-v3-watershed|v3 分水岭]]）。

## 与 Delta / Hudi / Paimon 的定位对比

四大湖仓表格式各有"娘家"（来源 [[iceberg-v3-watershed|v3 分水岭]]）：
...
> Paimon 详见 [[paimon]]（待建）。选型对比详见湖格式选型主题。
...
- Iceberg REST Catalog 协议细节（待 PDF [[iceberg-definitive-guide-dremio|Definitive Guide]] 逐页复核后补充）
- 写入路径与 Compaction 机制
- 与 Flink/Spark/Trino 集成的具体配置
